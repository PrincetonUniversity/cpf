...
void DevirtualizationAnalysis::studyModule(Module &mod)
{
  ...
  DEBUG(errs() << "There are " << candidates.size()
                                            ...
                                            const DataLayout &DL)
{
  ...
  {
    ++numFlowAnalyzed;
    DEBUG(errs() << "--> Flow-sensitive test found "
  ...
  DEBUG(errs() << "Possible targets of ``" << *cs.getInstruction()
               << "'' include:\n");
