...
namespace liberty
{
  ...
  {
    LLVM_LLVM_DEBUG(errs() << "Begin TypeAA::runOnModule()\n");
    ...
    currentMod = 0;
    LLVM_LLVM_DEBUG(errs() << "End TypeAA::runOnModule()\n");
    return false;
  }
  ...
  static bool isSingleCastFromMalloc(Instruction *inst)
  {
    ...
    if( src->getNumUses() - cmpCount - freeCount != 1 ) {
      LLVM_LLVM_DEBUG(errs() << "Use count not equal to 1\n");
      return false;
    }
    ...
    LLVM_LLVM_DEBUG(errs() << "Wasn't considered a call to malloc\n");
    return false;
  }
  ...
  static bool isCastFromMalloc(Instruction *inst)
  {
    ...
    if (castCount > 2) {
      LLVM_LLVM_DEBUG(errs() << "More than 2 casts for the malloc result\n");
      return false;
    }
  ...
  bool TypeSanityAnalysis::runOnFunction(Function &fcn)
  {
    ...
    if( fcn.isDeclaration() || (!FULL_UNIVERSAL && !isPrivate( fcn )) )
    {
      if( !isStupidSimpleCase(fcn) )
      {
        ...
        {
          LLVM_LLVM_DEBUG(errs() << "Conservative 1: " << fcn.getName() << " :: " <<  *fcn.getType() << '\n');
          addInsane( i->getType() );
        }
      }
    }
    ...
    if( fcn.isDeclaration() )
    {
      if( !isStupidSimpleCase(fcn) )
      {
        LLVM_LLVM_DEBUG(errs() << "Conservative 2: " << fcn.getName() << " :: " << *fcn.getType() << '\n');
    ...
    for(inst_iterator i=inst_begin(fcn), e=inst_end(fcn); i!=e; ++i)
    {
      ...
      if( isa<CastInst>( inst ) )
      {
        if( isSingleCastFromMalloc( inst ) )
        {
          ...
          LLVM_LLVM_DEBUG(errs() << "Is a single cast from malloc\n");
        }
        // sot
        else if( isCastFromMalloc ( inst ) )
        {
          ...
          LLVM_LLVM_DEBUG(errs() << "Inst " <<  *inst << "is a cast originating from malloc\n");
        }
        ...
        else
        {
          ...
          LLVM_LLVM_DEBUG(errs() << "Cast: " << *inst
                       << " at " << fcn.getName()
      ...
      {
        LLVM_LLVM_DEBUG(errs() << "VAArg: " << *inst << '\n');
      ...
      &&       cs.getCalledFunction()->isVarArg() )
      {
        ...
        {
          LLVM_LLVM_DEBUG(errs() << "Variadic call: " << *inst << '\n');
