...
public:
  ...
  {
    DEBUG(errs() << "NoCaptureGlobalAA::cannotAlias(\n"
                 << "  P1=" << *P1 << ",\n"
                 << "  P2=" << *P2 << ") ?\n");
    ...
    for(UO::iterator i=uo1.begin(), e=uo1.end(); i!=e; ++i)
    {
      ...
      if( ! gv )
      {
        DEBUG(errs() << "=> NO: P1 may refer to " << *object << ", which is not a global variable.\n");
        return false;
      }

      if( mayBeCaptured(gv) )
      {
        DEBUG(errs() << "=> NO: P1 might refer to the may-capture global variable " << *gv << '\n');
        return false;
      }
    }
    ...
    for(UO::iterator i=uo2.begin(), e=uo2.end(); i!=e; ++i)
    {
      ...
      else if( isa<LoadInst>(object) )
      {
        ...
        DEBUG(errs() << "   (Ok: P2 may refer to a load)\n");
        continue;
      }
      ...
      else if( const Argument *arg = dyn_cast< Argument >(object) )
      {
        ...
        if( arg->hasNoCaptureAttr() )
        {
          ...
          // in 'uo1'.
          DEBUG(errs() << "=> NO: P2 may refer to no-capture argument " << *arg << '\n');
          return false;
        }
        ...
        else
        {
          ...
          DEBUG(errs() << "   (Ok: P2 may refer to a may-capture argument " << *arg << ")\n");
          continue;
        }
      }
      ...
      else if( const GlobalValue *gv = dyn_cast< GlobalValue >(object) )
      {
        if( isa<GlobalAlias>(gv) )
        {
          ...
          DEBUG(errs() << "=> NO: Both P1,P2 may refer to " << *gv << '\n');
          return false;
        }
        ...
        else if( isNonCapturedGlobal(gv) )
        {
          ...
          else
          {
            ...
            DEBUG(errs() << "   (Ok: P2 and not P1 may refer to no-capture global " << *gv << ")\n");
            continue;
          }
        }
        else
        {
          ...
          DEBUG(errs() << "   (Ok: P2 may refer to may-capture global " << *gv << ")\n");
          continue;
        }
      }
      ...
      else
      {
        ...
        // object
        DEBUG(errs() << "=> NO: P2 may refer to " << *object << '\n');
        return false;
      }
    }
