...
bool PureFunAA::argumentsAlias(const ImmutableCallSite CS, const Value *P,
  ...
  LLVM_LLVM_DEBUG(errs() << "\t  Arguments do not alias\n");
  return false;
}
...
PureFunAA::PureFunAA() : ModulePass(ID), sccCount(0) {
  ...
  LLVM_LLVM_DEBUG(errs() << "Known pure functions: "  << pureFunSet.size()  << "\n");
  LLVM_LLVM_DEBUG(errs() << "Known local functions: " << localFunSet.size() << "\n");
}
...
PureFunAA::ModRefResult PureFunAA::getModRefInfo(CallSite CS1,
  ...
  LLVM_LLVM_DEBUG(errs() << "\tpure-fun-aa looking at " << *(CS1.getInstruction()) << " to " << *(CS2.getInstruction()) << "\n");
  ...
  if (isLocal(fun1) && isLocal(fun2) && !argumentsAlias(CS1, CS2, aa, TD, tmpR) &&
    ...
    LLVM_LLVM_DEBUG(errs() << "\t    pure-fun-aa returning NoModRef 1\n");
    for (auto remed : tmpR)
      R.insert(remed);
    return NoModRef;
  }
...
PureFunAA::ModRefResult PureFunAA::getModRefInfo(CallSite CS,
  ...
  LLVM_LLVM_DEBUG(errs() << "\tpure-fun-aa looking at " << *(CS.getInstruction()) << " to " << *Ptr << "\n");
  ...
  if(isLocal(fun) && !argumentsAlias(CS, Ptr, Size, AA, TD, tmpR) &&
    ...
    LLVM_LLVM_DEBUG(errs() << "\t    result of query "
                 << AA->alias(CS.getInstruction(), Size, Rel, Ptr, Size, L, tmpR)
                 << "\n");
    LLVM_LLVM_DEBUG(errs() << "\t    pure-fun-aa returning NoModRef 2\n");
    for (auto remed : tmpR)
      R.insert(remed);
    return NoModRef;
  }
