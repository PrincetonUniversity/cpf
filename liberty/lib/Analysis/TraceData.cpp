...
*/
  ...
  else if( Argument *arg = dyn_cast<Argument>(expr) )
  {
    ...
    if( context->hasInternalLinkage() && ! nocap.isCaptured(context) )
    {
      if( context->use_empty() )
        DEBUG(errs() << "*** Integer expr is an argument to a "
  ...
  DEBUG(errs() << "traceConcreteIntegerValues() failed on: ``"
               << *expr << "''\n");
  return false;
}
                  ...
                  const DataLayout &DL) const
{
  ...
  else if( Argument *arg = dyn_cast<Argument>(fcn_ptr) )
  {
    ...
    {
      if( context->use_empty() )
        DEBUG(errs() << "*** Called value is an argument to a "
                     << "function that is never called.  "
  ...
  DEBUG(errs() << "Flow-sensitive trace failed on: ``"
               << *fcn_ptr << "''\n");
  return false;
}
              ...
              const DataLayout &DL) const
{
  ...
  DEBUG(errs() << "Flow-sensitive trace load-from failed on: ``"
               << *ptr << "''\n");
  return false;
}
  ...
  Constant *constant, FcnList &output, ValueSet &already) const
{
  ...
  DEBUG(errs() << "Flow-sensitive trace load-from-constant failed on: ``"
               << *constant << "''\n");
  return false;
}
