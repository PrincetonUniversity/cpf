...
protected:
  ...
  // allocate 8 bytes of metadata per one byte of data, but changes to
  // shadow memory needed
  static constexpr uint32_t LOCALWRITE_SHIFT = 0;
  // takes in a lambda action and uint64_t addr
  template <typename F>
  inline void local_write(uint64_t value, const F &action) {
    if (((value >> LOCALWRITE_SHIFT) & LOCALWRITE_MASK) == LOCALWRITE_PATTERN) {
      action();
    }
  }

public:
  GenericLocalWriteModule(uint32_t mask, uint32_t pattern)
      : LOCALWRITE_MASK(mask), LOCALWRITE_PATTERN(pattern) {}
  virtual ~GenericLocalWriteModule() = default;
};
