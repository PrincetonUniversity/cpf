...
void SLAMP_init(uint32_t fn_id, uint32_t loop_id) {
  int rv;
  int bytes;
  char *url = "ipc:///tmp/SLAMP.ipc";
  if ((rv = nng_push0_open(&sock)) != 0) {
    fprintf(stderr, "nng_push0_open: %s", nng_strerror(rv));
    exit(1);
  }
  if ((rv = nng_dial(sock, url, nullptr, 0)) != 0) {
    fprintf(stderr, "nng_dial: %s", nng_strerror(rv));
    exit(1);
  }
  // set NN_SNDBUF to 100MB
  int sndbuf = 8192;
  if ((rv = nng_setopt_int(sock, NNG_OPT_SENDBUF, sndbuf)) != 0) {
    fprintf(stderr, "nng_setopt_int: %s", nng_strerror(rv));
    exit(1);
  }
  ...
  if ((rv = nng_send(sock, msg, strlen(msg), 0)) != 0) {
    fprintf(stderr, "nng_send: %s", nng_strerror(rv));
    exit(1);
  }
}
...
void SLAMP_load1(uint32_t instr, const uint64_t addr, const uint32_t bare_instr, uint64_t value){
  int rv;
  char buf[1024];
  // msg = instr + addr + value
  sprintf(buf, "%d %lu %lu", instr, addr, value);

  if ((rv = nng_send(sock, buf, strlen(buf), 0)) != 0) {
    fprintf(stderr, "nng_send: %s", nng_strerror(rv));
    exit(1);
  }
}
