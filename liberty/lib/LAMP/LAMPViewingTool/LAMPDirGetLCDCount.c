...
unsigned int buildTable (unsigned lconly);
...
void countNumberOfLCDsInEachLoop();
...
void countNumberOfLCDsInEachLoop() {
   ...
   while(lastID != loopID)
   {
     unsigned int numLCDs = buildTable(loopID);
     fprintf (stdout, "%d loop-carried dependences in Loop (ID, line, fn) = (%d, %d, %s)\n", numLCDs, loopID, transLoopLine, fn);
     lastID = loopID;
     fscanf(inFile, "%d%d%s", &loopID, &transLoopLine, fn);
   }
...
unsigned int readLoopTranslator(int loopLine)
{
   FILE *inFile;
   int loopID, transLoopLine, lastID = -1;
   int check;
...
/*AR: Modified to return the number of LCDs in the loop*/
unsigned int buildTable(unsigned loop)
{
  char comment;
