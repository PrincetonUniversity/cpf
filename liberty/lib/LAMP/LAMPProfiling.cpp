...
namespace {
  class LAMPProfiler : public FunctionPass {
    ...
    {
      LLVM_LLVM_DEBUG(errs() << "In LAMPProfiler ctor\n");
...
*/
  ...
  if (F.getName().str() == "_GLOBAL__I_step_number" || F.getName().str() == "___tspec_destructor" ) {
    LLVM_LLVM_DEBUG(errs() << "Skipping Function " << F.getName().str() << " beginning ID: " << instruction_id << '\n');
    return true;
  }
  ...
  for (Function::iterator IF = F.begin(), IE = F.end(); IF != IE; ++IF)
  {
    ...
    for (BasicBlock::iterator I = BB.begin(), E = BB.end(); I != E; ++I, ++iid)
    {
      ...
      if (isa<LoadInst>(I))
      {
        ...
        CHECK_INST(I, lim)
        {
          LLVM_LLVM_DEBUG(errs() << "tLAMP skipping inst: " << I << ":" << *I << "\n");
          continue;
        }
#endif
      ...
      else if (isa<StoreInst>(I))
      {
        ...
        LLVM_LLVM_DEBUG(errs() << ptr->getName().str() << "\n");
        Args[1] = new PtrToIntInst(ptr, Type::getInt64Ty(M->getContext()), "addr_var", &*I);
      ...
      //LLVM_LLVM_DEBUG(errs() << "Source: " << *SrcTy);
      ...
      CallInst::Create(DeallocFn, Args, "", I);
      LLVM_LLVM_DEBUG(errs() << instruction_id << *I);
  ...
  LLVM_LLVM_DEBUG(errs() << "Instrumentation of " << F.getName().str() << " complete.  Ending ID: " << instruction_id << '\n');
...
bool LAMPInit::runOnModule(Module& M)
{
  for(Module::iterator IF = M.begin(), E = M.end(); IF != E; ++IF)
  {
    ...
    if (F.getName().str() == InitFunctionName) {
      ...
      LLVM_LLVM_DEBUG(errs() << "LAMP-- Ld/St/Intrinsic/Call Count:" << cnt << " Loop Count:" << lps <<'\n');
  ...
  return false;
    ...
    bool LAMPLoopProfiler::runOnLoop(Loop *Lp, LPPassManager &LPM) {
      ...
      if(TLAMP)
      {
        ...
        {
          LLVM_LLVM_DEBUG(errs() << "Loop " << header->getName() << " is not limiting\n");
      ...
      LLVM_LLVM_DEBUG(errs() << "Num Loops Processed: " << numLoops << "  Loop ID: " << loop_id << '\n');
