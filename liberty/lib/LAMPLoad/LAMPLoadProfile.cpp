      ...
      {    // for all instructions in a block
        for (BasicBlock::iterator IB = BBB->begin(), IE = BBB->end(); IB != IE; IB++)
        {
          ...
          { // count loads, stores, calls
            ...
            DEBUG(errs() << "Adding instruction -" << *IB << "- with ID:" << lamp_id << "\n");
...
//                (dyn_cast<CallInst>(IB)->getCalledFunction()->isDeclaration())))
  ...
  /*
     ...
     Loop *L = *LB;
     DEBUG(errs() << lamp_id << *(L->getLoopPreheader()));
     }
     }
  ...
  /*
     ...
     DEBUG(errs() << (*iter2).first << " "  << (*iter2).second  << '\n');
     i++;
     }
     DEBUG(errs() << i << '\n');
   */
  ...
  DEBUG(errs() << "Opened " << ProfileFileName << "\n");
                ...
                if (myI && (dyn_cast<User>(myI))->getNumOperands() != 0)
                {
                  ...
                  else
                  {
                    ...
                    {  // should recurse in multiple directions here
                      // myV = myI->getOperand(0);  // try it anyway ???
                      //myI = dyn_cast<Instruction>(myI->getOperand(0));
                      DEBUG(errs() << "attempting zrecurse" << i << "\t" << i1_id << "\n");
                      needCheck++;
                      globLoop = loop_id;
                ...
                if (myI && (dyn_cast<User>(myI))->getNumOperands() != 0)
                {
                  ...
                  else
                  {
                    ...
                    {  // should recurse in multiple directions here
                      //myV = myI->getOperand(0);  // try it anyway ???
                      //myI = dyn_cast<Instruction>(myI->getOperand(0));
                      DEBUG(errs() << "attempting recurse" << i++ << "\t" << i2_id << "\n");
                      globLoop = loop_id;
                      ISet avoidInfiniteRecursion;
...
//            }
            ...
            /*
               ...
               DEBUG(errs() << "~I think the pair pointers are equal\n");
               } else {
               DEBUG(errs() << "~The pointers are not equal\n");
               DEBUG(errs() << "Ptr 1: " << dep_inst_pair_ptr->first  << " " << dep_inst_pair_ptr->second << "\n");
               DEBUG(errs() << "Ptr 2: " << dep_inst_pair_ptr2->first << " " << dep_inst_pair_ptr2->second << "\n");
               }

               InstPairSet::iterator siter;
               siter = LoopToDepSetMap[BB].find(*dep_inst_pair_ptr);
               if(siter == LoopToDepSetMap[BB].end() ) {
               DEBUG(errs() << "~~~did NOT find instruction pair we just inserted\n");

               } else {
               DEBUG(errs() << "~~~FOUND instruction pair we just inserted\n");


               }
               siter = LoopToDepSetMap[BB].find(*dep_inst_pair_ptr2);
               if(siter == LoopToDepSetMap[BB].end() ) {
               DEBUG(errs() << "~~~did NOT find duplicate pair\n");

               } else {
               DEBUG(errs() << "~~~FOUND duplicate pair\n");
                  ...
                  if (myI && (dyn_cast<User>(myI))->getNumOperands() != 0)
                  {
                    ...
                    else
                    {
                      ...
                      if ((dyn_cast<User>(myI))->getNumOperands() == 1)
                      {
                        ...
                        DEBUG(errs() << "DDOK" << i << "\t" << i1_id << "\n");
                      }
                      else
                      {  // should recurse in multiple directions here
                        //myV = myI->getOperand(0);  // try it anyway ???
                        //myI = dyn_cast<Instruction>(myI->getOperand(0));
                        //DEBUG(errs() << "DDfail" << i << "\t" << i1_id << "\n");

                        DEBUG(errs() << "DDattempting recurse" << j++ << "\t" << i1_id << "\n");
                        globLoop = loop_id;
                        ISet avoidInfiniteRecursion;
                        recurseOperands(myI, temp1->getParent()->getParent()->getName().str(),
                            temp1->getParent()->getName().str(), 1, ln1,avoidInfiniteRecursion);
                  ...
                  if (myI && (dyn_cast<User>(myI))->getNumOperands() != 0)
                  {
                    ...
                    else
                    {
                      if ((dyn_cast<User>(myI))->getNumOperands() == 1)
                      {
                        myV = myI->getOperand(0);  // try it anyway ???
                        myI = dyn_cast<Instruction>(myI->getOperand(0));
                        DEBUG(errs() << "DDOK" << i << "\t" << i2_id << "\n");
                      }
                      else
                      {  // should recurse in multiple directions here
                        //myV = myI->getOperand(0);  // try it anyway ???
                        //myI = dyn_cast<Instruction>(myI->getOperand(0));
                        //DEBUG(errs() << "DDfail" << i << "\t" << i2_id << "\n");

                        DEBUG(errs() << "attempting recurse" << j++ << "\t" << i2_id << "\n");
                        globLoop = loop_id;
                        ISet avoidInfiniteRecursion;
                        recurseOperands(myI, temp2->getParent()->getParent()->getName().str(),
                            temp2->getParent()->getName().str(), 2, ln2,avoidInfiniteRecursion);
...
//              }
  ...
  DEBUG(errs() << "Num of cross-dep Loops: "<< LoopToDepSetMap.size() << '\n');
  /*
  ...
  DEBUG(errs() << "LoopToIDMap_global[Liter->frist]   max_times   (Id1, Id2)\n");
  DEBUG(errs() << LoopToIdMap_global[Liter->first] << " " << max_times
               << " ("  << Id1 << "," << Id2 << ")"<< '\n');
...
void recurseOperands(Instruction * myI, std::string fnName, std::string bbName, int list, unsigned line, ISet &avoidInfiniteRecursion)
{
  ...
  {
    DEBUG(errs() << "Crazy Recursion attempt " << (dyn_cast<User>(myI))->getNumOperands() << "\n\n");
    ...
    if (walk1->name != "")
      while(walk2)
      {
        if (walk2->name != "")
        {
          ...
          DEBUG(errs() << "FOUND: " << walk1->name << " -- " << walk2->name << "\n");
        }
