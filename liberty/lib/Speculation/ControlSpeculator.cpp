...
bool ProfileGuidedControlSpeculator::dominatesTargetHeader(const BasicBlock* bb)
{
  ...
  if ( dt.dominates( bb, targetHeader ) ) {
    LLVM_LLVM_DEBUG(errs() << "bb " << bb->getName() << " dominates target header "
...
void ProfileGuidedControlSpeculator::visit(const Function *fcn)
{
  ...
  LLVM_LLVM_DEBUG(errs() << "CtrlSpec: visit( " << fcn->getName() << " )\n");
  ...
  {
    LLVM_LLVM_DEBUG(errs() << "CtrlSpec: function does not have profile data avaiable\n");
        ...
        if (MDS->getString().equals("function_entry_count")) {
          ...
          if (Count == 0) {
            ...
            for (Function::const_iterator i = fcn->begin(), e = fcn->end();
              ...
              LLVM_LLVM_DEBUG(errs() << "CtrlSpec " << fcn->getName() << ": block "
  ...
  {
    LLVM_LLVM_DEBUG(errs() << "CtrlSpec: function never executed\n");
    ...
    for(unsigned sn=0; sn<N; ++sn)
    {
      ...
      else
      {
        ...
        if (!loopOfInterestExit && rate > pred_cnt * MaxMisspec) {
          LLVM_LLVM_DEBUG(errs() << pred->getName() << "->" << succ->getName() << ", "
                       << (unsigned)rate << " > " << (unsigned)pred_cnt << " * "
                       << format("%f", MaxMisspec) << "\n");
          continue;
        }
      ...
      LLVM_LLVM_DEBUG(errs() << "CtrlSpec " << fcn->getName()
