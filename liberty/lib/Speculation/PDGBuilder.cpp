...
std::unique_ptr<llvm::PDG> llvm::PDGBuilder::getLoopPDG(Loop *loop) {
  ...
  LLVM_LLVM_DEBUG(errs() << "constructEdgesFromMemory with CAF ...\n");
  ...
  LLVM_LLVM_DEBUG(errs() << "constructEdgesFromMemory with SCAF ...\n");
  aa->dump();
  constructEdgesFromMemory(*pdg, loop, aa);
  removeSpecModulesFromLoopAA();
  //LLVM_LLVM_DEBUG(errs() << "revert stack to CAF ...\n");
  //aa->dump();

  LLVM_LLVM_DEBUG(errs() << "constructEdgesFromControl ...\n");

  //auto *F = loop->getHeader()->getParent();
  //auto &PDT = getAnalysis<PostDominatorTreeWrapperPass>(*F).getPostDomTree();
  constructEdgesFromControl(*pdg, loop);

  LLVM_LLVM_DEBUG(errs() << "constructEdgesFromUseDefs ...\n");

  // constructEdgesFromUseDefs adds external nodes for live-ins and live-outs
  constructEdgesFromUseDefs(*pdg, loop);

  LLVM_LLVM_DEBUG(errs() << "PDG construction completed\n");
...
void llvm::PDGBuilder::constructEdgesFromMemory(PDG &pdg, Loop *loop,
  ...
  LLVM_LLVM_DEBUG(errs() << "Total memory dependence queries to CAF: " << memDepQueryCnt
               << "\n");
}
