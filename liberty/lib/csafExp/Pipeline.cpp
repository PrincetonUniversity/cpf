...
//#include "liberty/SpecPriv/Complaints.h"
...
static void printPipeline(const Loop *loop, const PDG &pdg, const SCCs &sccs, const PipelineStrategyOLD &strat, ControlSpeculation *ctrlspec = 0)
{
  if( !PrintPipeline )
    return;
  const BasicBlock *header = loop->getHeader();
...
static EdgeWeight estimate_weight(const PDG &pdg, const SCCs &sccs, PerformanceEstimatorOLD &perf, unsigned scc_id)
{
...
static EdgeWeight estimate_weight(const PDG &pdg, const SCCs &sccs, PerformanceEstimatorOLD &perf, const PipelineStageOLD &stage)
{
  return perf.estimate_weight( stage.instructions.begin(), stage.instructions.end() );
}

static EdgeWeight estimate_weight(const PDG &pdg, const SCCs &sccs, PerformanceEstimatorOLD &perf, const SCCs::SCCSet &scc_set)
{
  EdgeWeight sum_weight = 0;
...
bool Pipeline::suggest(
  Loop *loop, LoopAA *loopAA,
  //Remediator *remed,
  ControlSpeculation &ctrlspec, PredictionSpeculation &predspec,
  PerformanceEstimatorOLD &perf,
  PipelineStrategyOLD &strat,
  unsigned threadBudget,
  bool ignoreAntiOutput, bool includeReplicableStages, bool constrainSubLoops, bool abortIfNoParallelStage,
  bool includeParallelStages)
{
  ...
  // try to simplify the pdg with remedies
  //pdg.setRemed(remed);
  //remed->dump();

  /*
  // create complaints and use remediator stack to handle them
  errs() << "\t*** Creating and addressing complaints\n";

  bool improved = Complaints::createAndAddressComplaints(loop, pdg, sccs, perf, strat);
  bool pstageAfterRemedies = false;
  if (improved) {
      DEBUG(errs() << "Addressed complaints led to simplified PDG\n");
      // recompute the DagSCC with the updated PDG
      // do not abort if no parallel stage
      //pstageAfterRemedies = SCCs::computeDagScc(pdg, sccs, abortIfNoParallelStage);
      sccs.recompute(pdg);
      // just to mark new sccs as parallel or sequential (could be done more easily TODO: create separate function)
      pstageAfterRemedies = SCCs::computeDagScc_exhaustive(pdg, sccs, abortIfNoParallelStage);
  }
  */
  ...
  if( !pstage )
  {
    ...
    strat.stages.push_back( PipelineStageOLD(loop) );
    return false;
  }
  ...
  bool includeParallelStages)
{
  ...
  {
    const PipelineStageOLD &si = strat.stages[i];
    // Foreach instruction src in that stage that may source control deps
    for(PipelineStageOLD::ISet::iterator j=si.instructions.begin(), z=si.instructions.end(); j!=z; ++j)
    {
      Instruction *src = *j;
...
//        if( !si.instructions.count(dst) )
          strat.crossStageDeps.push_back( CrossStageDependenceOLD(src,dst, pes.find(vsrc,vdst) ) );
      }
    }
  }
...
bool Pipeline::doallAndPipeline(
  ...
  PipelineStrategyOLD::Stages &stages, unsigned threadBudget, bool includeReplicableStages,
  bool includeParallelStages)
{
  // Set of all SCCs
  ...
  if( stages.size() == 1
  &&  stages[0].type != PipelineStageOLD::Parallel )
    return false;
  ...
  unsigned threadBudget, bool includeReplicableStages, bool includeParallelStages)
{
  ...
  if( includeParallelStages && findMaxParallelStage(pdg,sccs,all_sccs,perf,ctrlspec,includeReplicableStages,maxParallel,notMaxParallel) )
  {
    ...
    for(int nbefore=min_before; nbefore<=max_before; ++nbefore)
    {
      ...
      myStages.push_back( PipelineStageOLD(PipelineStageOLD::Parallel, pdg, sccs, maxParallel) );
      ...
      for(int nafter=min_after; nafter<=max_after; ++nafter)
      {
        ...
        for(int npar=max_par; npar>=min_par; --npar)
        {
          ...
          PipelineStrategyOLD::Stages expanded = myStages; // copy
          PipelineStrategyOLD::expandReplicatedStages( expanded );
  ...
  if( includeReplicableStages )
  {
    ...
    if( findMaxReplicableStage(pdg,sccs,all_sccs,perf,maxReplicable,notMaxReplicable) )
    {
      ...
      for(int nbefore=max_before; nbefore>=min_before; --nbefore)
      {
        ...
        myStages.push_back( PipelineStageOLD(PipelineStageOLD::Replicable, pdg, sccs, maxReplicable) );
...
//              errs() << "+ + + + Decrease key for successor SCC #" << other_scc << " from " << i << " to " << (i-1) << ".\n";
      ...
      PipelineStageOLD seq( PipelineStageOLD::Sequential, pdg, sccs, stage_sccs );
  ...
  PipelineStageOLD seq = PipelineStageOLD( PipelineStageOLD::Sequential, pdg, sccs, all_sccs);
  stages.push_back(seq);
  ...
  bool includeParallelStages)
{
  BasicBlock *header = loop->getHeader();
  Function *fcn = header->getParent();
  errs() << "Pipeline::isApplicable(" << fcn->getName() << "::" << header->getName() << "):\n";

  PipelineStrategyOLD strat;
  if( suggest(loop,loopAA,
        //remed,
        ctrlspec,predspec,perf,strat,threadBudget,ignoreAntiOutput,includeReplicableStages,constrainSubLoops, abortIfNoParallelStage, includeParallelStages) )
    return true;
