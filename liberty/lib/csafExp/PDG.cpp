...
STATISTIC(numRegDepsCutBecauseReduction, "Num reg deps cut because reduction detected");
...
: numQueries(0), V(v), E(V), ctrlspec(cs), aa(AA), ignoreAntiOutput(ignore)
...
: numQueries(0), V(v), E(V), ctrlspec(cs), ignoreAntiOutput(ignore)
    ...
    for(User::const_op_iterator j=user->op_begin(), z=user->op_end(); j!=z; ++j)
    {
      ...
      if( const Instruction *src = dyn_cast<Instruction>(operand) )
        if( V.count(src) )
        {
         ...
         // sum, min/max reduction is now detected through remediators
         /*
         // missing LLVM isReductionPHI
          if (reduxdet.isSumReduction(loop, src, user, loopCarried))
          {
            ++numRegDepsCutBecauseReduction;
            continue;
          }

          // Perhaps this is a minmax reduction
...
/*
bool PDG::tryRemoveMemEdge(Vertices::ID src, Vertices::ID dst,
  ...
  // forward dep test
  const LoopAA::ModRefResult forward = remed->modref(sop, FW, dop, loop);
  if (LoopAA::NoModRef == forward)
    return true;
  ...
  if (FW != RV || sop != dop) {
    // reverse dep test
    reverse = remed->modref(dop, RV, sop, loop);
...
bool PDG::tryRemoveLoopCarriedMemEdge(Vertices::ID src, Vertices::ID dst) {
  return tryRemoveMemEdge(src, dst, LoopAA::Before, LoopAA::After);
}

bool PDG::tryRemoveLoopCarriedRegEdge(Vertices::ID src, Vertices::ID dst) {
  ++numComplaints;
  ...
  if ( resp.result == Remediator::NoDep){
    errs() << "Register Dependence Removed!" << "\n";
    errs() << "Remediator ID: " << resp.info.remediatorID << "; Remedy ID: " << resp.info.remedyID << "; Cost: " << resp.info.cost << "\n";
    return true;
  }
  return false;
}

bool PDG::tryRemoveLoopCarriedCtrlEdge(Vertices::ID src, Vertices::ID dst) {
  ++numComplaints;
  ...
  if ( resp.result == Remediator::NoDep){
    ...
    errs() << "Control Dependence Removed!" << "\n";
    errs() << "Remediator ID: " << resp.info.remediatorID << "; Remedy ID: " << resp.info.remedyID << "; Cost: " << resp.info.cost << "\n";
    return true;
  }
  return false;
...
bool PDG::tryRemoveIntraIterationMemEdge(Vertices::ID src, Vertices::ID dst) {
  return tryRemoveMemEdge(src, dst, LoopAA::Same, LoopAA::Same);
}

bool PDG::tryRemoveIntraIterationRegEdge(Vertices::ID src, Vertices::ID dst) {
  ++numComplaints;
  ...
  Remediator::RemedResp resp = remed->regdep(sop, dop, false, loop);
  //Remediator::DepResult res = remed->regdep(sop, dop, true, loop);
...
bool PDG::tryRemoveIntraIterationCtrlEdge(Vertices::ID src, Vertices::ID dst) {
  ++numComplaints;
...
/*
bool PDG::tryRemoveAllLimitingEdges(Complaints complaints) {
  Remediator::RemedResp resp = remed->limitingDeps(complaints, this);
  if ( resp.result == Remediator::NoDep){
    errs() << "All remaining edges removed" << "\n";
    errs() << "Remediator ID: " << resp.info.remediatorID << "; Remedy ID: " << resp.info.remedyID << "; Cost: " << resp.info.cost << "\n";
    return true;
  }
  return false;
...
void PDG::removeLoopCarriedMemEdge(Vertices::ID src,
                                               Vertices::ID dst) {
  E.removeLCMem(src, dst);
}

void PDG::removeLoopCarriedCtrlEdge(Vertices::ID src,
                                                Vertices::ID dst) {
  E.removeLCCtrl(src, dst);
}

void PDG::removeLoopCarriedRegEdge(Vertices::ID src,
                                               Vertices::ID dst) {
  E.removeLCReg(src, dst);
}

void PDG::removeIntraIterationMemEdge(Vertices::ID src,
                                               Vertices::ID dst) {
  E.removeIIMem(src, dst);
}

void PDG::removeIntraIterationCtrlEdge(Vertices::ID src,
                                                Vertices::ID dst) {
  E.removeIICtrl(src, dst);
}

void PDG::removeIntraIterationRegEdge(Vertices::ID src,
                                               Vertices::ID dst) {
...
PDG::PDG(const Vertices &vertices, PartialEdgeSet &edges, ControlSpeculation &cs, bool ignoreAO, LoopAA *stack)
  : V(vertices), E(edges), ctrlspec(cs), ignoreAntiOutput(ignoreAO), aa(stack) {}
