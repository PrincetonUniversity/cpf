...
#ifndef LLVM_LIBERTY_SPEC_PRIV_PERFORMANCE_ESTIMATOR_OLD_H
#define LLVM_LIBERTY_SPEC_PRIV_PERFORMANCE_ESTIMATOR_OLD_H
...
{
  virtual ~PerformanceEstimatorOLD() {}
  ...
  virtual unsigned long estimate_pipeline_weight(const PipelineStrategyOLD::Stages &stages);
  virtual unsigned long estimate_pipeline_weight(const PipelineStrategyOLD::Stages &stages, const Loop *loop);
  unsigned long estimate_pipeline_weight(const PipelineStrategyOLD &strategy)
    { return estimate_pipeline_weight( strategy.stages ); }
  unsigned long estimate_pipeline_weight(const PipelineStrategyOLD &strategy, const Loop *loop)
    { return estimate_pipeline_weight( strategy.stages, loop ); }

};

/// This is the 'dumb' implementation of a performance estimator.
struct FlatPerformanceEstimatorOLD : public PerformanceEstimatorOLD
{
