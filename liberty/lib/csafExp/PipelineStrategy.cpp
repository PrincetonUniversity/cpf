...
void PipelineStageOLD::print_inst_txt(raw_ostream &fout, ControlSpeculation *ctrlspec, Instruction *inst, StringRef line_suffix) const
{
  fout << "    ";
...
bool PipelineStageOLD::communicatesTo(const PipelineStageOLD &other) const
{
  // TODO
  return true;
}

PipelineStageOLD::PipelineStageOLD(Loop *loop)
  : type( PipelineStageOLD::Sequential ), parallel_factor(1)
{
...
PipelineStageOLD::PipelineStageOLD(Type t, const PDG &pdg, const SCCs &sccs, const SCCs::SCCSet &scc_list)
  : type(t), parallel_factor(1)
{
  const Vertices &V = pdg.getV();
  for(unsigned i=0, N=scc_list.size(); i<N; ++i)
  {
    const SCCs::SCC &scc = sccs.get( scc_list[i] );

    for(SCCs::SCC::const_iterator j=scc.begin(), z=scc.end(); j!=z; ++j)
      instructions.insert( V.get(*j) );
  }
}

void PipelineStageOLD::print_txt(raw_ostream &fout, ControlSpeculation *ctrlspec, StringRef line_suffix) const
{
  if( ! replicated.empty() )
  {
    ...
    for(ISet::const_iterator i=replicated.begin(), e=replicated.end(); i!=e; ++i)
      print_inst_txt(fout, ctrlspec, *i, line_suffix);
  }
...
void PipelineStrategyOLD::assertPipelineProperty(const PDG &pdg) const
{
  ...
  {
    const PipelineStageOLD &earlier = *i;
    for(Stages::const_iterator j=i+1; j!=e; ++j)
    {
      const PipelineStageOLD &later = *j;
  ...
  for(Stages::const_iterator i=stages.begin(), e=stages.end(); i!=e; ++i)
  {
    const PipelineStageOLD &pstage = *i;
    if( pstage.type != PipelineStageOLD::Parallel )
      continue;
...
/*
    ...
    {
      const PipelineStageOLD &other = *j;
...
// in 'other'
void PipelineStrategyOLD::assertParallelStageProperty(const PDG &pdg, const PipelineStageOLD &parallel, const PipelineStageOLD &other) const
{
  const Vertices &V = pdg.getV();

  for(PipelineStageOLD::ISet::const_iterator i=parallel.instructions.begin(), e=parallel.instructions.end(); i!=e; ++i)
  {
    Instruction *p = *i;
    Vertices::ID pid = V.get(p);

    for(PipelineStageOLD::ISet::const_iterator j=other.instructions.begin(), z=other.instructions.end(); j!=z; ++j)
    {
      Instruction *q = *j;
...
void PipelineStrategyOLD::assertPipelineProperty(const PDG &pdg, const PipelineStageOLD &earlyStage, const PipelineStageOLD &lateStage) const
{
  const Vertices &V = pdg.getV();

  PipelineStageOLD::ISet all_early = earlyStage.instructions;
  all_early.insert( earlyStage.replicated.begin(), earlyStage.replicated.end() );

  PipelineStageOLD::ISet all_late = lateStage.instructions;
  all_early.insert( lateStage.replicated.begin(), lateStage.replicated.end() );

  // For each operation from an earlier stage
  for(PipelineStageOLD::ISet::const_iterator i=all_early.begin(), e=all_early.end(); i!=e; ++i)
  {
    Instruction *early = *i;
    Vertices::ID eid = V.get(early);

    // For each operation from a later stage
    for(PipelineStageOLD::ISet::const_iterator j=all_late.begin(), z=all_late.end(); j!=z; ++j)
    {
      Instruction *late = *j;
...
void PipelineStrategyOLD::assertCheckedPipelineProperty(const PDG &pdg, const PipelineStageOLD &earlyStage, const PipelineStageOLD &lateStage) const
{
  const Vertices &V = pdg.getV();
...
void PipelineStrategyOLD::dump_pipeline(raw_ostream &fout, ControlSpeculation *ctrlspec, StringRef line_suffix) const
{
  if( !isValid() )
  {
  ...
  for(unsigned i=0, N=stages.size(); i<N; ++i)
  {
    ...
    stage.print_txt(fout, ctrlspec, line_suffix);
  }
  ...
  for(PipelineStrategyOLD::CrossStageDependenceOLDs::const_iterator i=crossStageDeps.begin(), e=crossStageDeps.end(); i!=e; ++i)
  {
    const CrossStageDependenceOLD &dep = *i;
...
bool PipelineStrategyOLD::expandReplicatedStages()
{
  return expandReplicatedStages( this->stages );
}

bool PipelineStrategyOLD::expandReplicatedStages(Stages &stages)
{
  ...
  for(int i=N-1; i>=0; --i)
  {
    PipelineStageOLD &replicable = stages[i];
    if( replicable.type != PipelineStageOLD::Replicable )
      continue;
    ...
    for(int j=i+1; j<N; ++j)
    {
      PipelineStageOLD &nonReplicable = stages[j];
      if( nonReplicable.type == PipelineStageOLD::Replicable )
        continue;
    ...
    // make it sequential
    if( numUsers < 1 )
      replicable.type = PipelineStageOLD::Sequential;
  }

  // Remove *all* replicable stages from the pipeline.
  unsigned put=0;
  for(int i=0; i<N; ++i)
    if( stages[i].type != PipelineStageOLD::Replicable )
      stages[put++] = stages[i];
...
void PipelineStrategyOLD::summary(raw_ostream &fout) const
{
  fout << "DSWP[";
  for(unsigned i=0, N=stages.size(); i<N; ++i)
  {
    ...
    {
      case PipelineStageOLD::Sequential:
        fout << 'S';
        break;
      case PipelineStageOLD::Replicable:
        fout << 'R';
        break;
      case PipelineStageOLD::Parallel:
...
void PipelineStrategyOLD::addInstruction(Instruction *newInst, Instruction *gravity)
{
  bool added = false;
  ...
  for(unsigned i=0, N=stages.size(); i<N; ++i)
  {
    PipelineStageOLD &stage = stages[i];
  ...
  {
    const CrossStageDependenceOLD &dep = crossStageDeps[i];
    if( dep.edge.lc_ctrl || dep.edge.ii_ctrl )
      if( dep.dst == gravity )
      {
        crossStageDeps.push_back( CrossStageDependenceOLD(dep.src, newInst, dep.edge) );
        added = true;
      }
  }
...
void PipelineStrategyOLD::replaceInstruction(Instruction *newInst, Instruction *oldInst)
{
  bool replaced = false;
  ...
  for(unsigned i=0, N=stages.size(); i<N; ++i)
  {
    ...
    PipelineStageOLD::ISet::iterator ff = stage.replicated.find(oldInst);
  ...
  {
    CrossStageDependenceOLD &dep = crossStageDeps[i];
    if( oldInst == dep.src )
    {
      dep.src = newInst;
      replaced = true;
...
void PipelineStrategyOLD::getExecutingStages(Instruction* inst, std::vector<unsigned>& executing_stages)
{
  for (unsigned i=0, N=stages.size() ; i<N ; ++i)
  {
...
bool PipelineStrategyOLD::ifI2IsInI1IsIn(Instruction *i1, Instruction *i2)
{
  assert( i1 && i2 );
...
void LoopParallelizationStrategyOLD::assertConsistentWithIR(Loop *loop)
{
...
void PipelineStrategyOLD::assertConsistentWithIR(Loop *loop)
{
  bool okay = true;

  // common invariants
  LoopParallelizationStrategyOLD::assertConsistentWithIR(loop);
  ...
  for(unsigned i=0, N=stages.size(); i<N; ++i)
  {
    PipelineStageOLD::ISet &stage = stages[i].instructions;
    for(PipelineStageOLD::ISet::const_iterator j=stage.begin(), z=stage.end(); j!=z; ++j)
    {
      Instruction *inst = *j;
      if( all_stages.count(inst) )
      {
        ...
        errs() << "PipelineStrategyOLD inconsistent with IR for loop "
  ...
  // stage.
  {
    ...
    for(unsigned i=0, N=stages.size(); i<N; ++i)
    {
      PipelineStageOLD::ISet &prefix = stages[i].replicated;
      for(PipelineStageOLD::ISet::const_iterator j=prefix.begin(), z=prefix.end(); j!=z; ++j)
      {
        Instruction *inst = *j;
  ...
  for(CrossStageDependenceOLDs::const_iterator i=crossStageDeps.begin(), e=crossStageDeps.end(); i!=e; ++i)
  {
    const CrossStageDependenceOLD &dep = *i;
...
void PipelineStrategyOLD::print_dot(raw_ostream &fout, const PDG &pdg, const SCCs &sccs, ControlSpeculation *ctrlspec) const
{
  const Vertices &V = pdg.getV();
  ...
  {
    const PipelineStageOLD &stage = stages[stageno];
    ...
    if( stage.type == PipelineStageOLD::Sequential )
      fout << "  color=pink;\n";
    else
...
void PipelineStrategyOLD::print_dot(const PDG &pdg, const SCCs &sccs, StringRef dot, StringRef tred, ControlSpeculation *ctrlspec) const
{
  {
    std::error_code ec;
...
bool PipelineStrategyOLD::mayExecuteInStage(const Instruction *inst, unsigned stageno) const
{
  const PipelineStageOLD &stage = stages[stageno];
...
bool PipelineStrategyOLD::mayExecuteInParallelStage(const Instruction *inst) const
{
  for(unsigned stageno=0, N=stages.size(); stageno<N; ++stageno)
    if( stages[stageno].type == PipelineStageOLD::Parallel )
      if( mayExecuteInStage(inst,stageno) )
        return true;
  return false;
}

bool PipelineStrategyOLD::maybeAntiPipelineDependence(const Instruction *src, const Instruction *dst) const
{
...
bool PipelineStrategyOLD::maybeAntiParallelStageDependence(const Instruction *src, const Instruction *dst) const
{
