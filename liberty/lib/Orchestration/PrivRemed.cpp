                                 ...
                                 const Criticisms &criticisms)
{
  ...
  // print number
  LLVM_LLVM_DEBUG(errs() << "Number of RAW collab deps handled by PrivRemed: " << RAWcollabDepsHandled << '\n');
  LLVM_LLVM_DEBUG(errs() << "Number of WAW collab deps handled by PrivRemed: " << WAWcollabDepsHandled << '\n');
...
bool PrivRemediator::instMustKill(const Instruction *inst, const Value *ptr,
  ...
  if (mustAlias(killptr, ptr)) {
    LLVM_LLVM_DEBUG(errs() << "There can be no loop-carried flow mem deps to because "
                    "killed by "
  ...
  if (auto gv = dyn_cast<GlobalValue>(ptr)) {
    ...
    if (!localGVs.count(gv)) {
      if (!isGlobalLocalToLoop(gv, L))
        return false;
      localGVs.insert(gv);
      LLVM_LLVM_DEBUG(errs() << "Global found to be local: " << *gv << "\n");
    }
    ...
    if (isPrivate(I, L, ctrlSpecUsed)) {
      LLVM_LLVM_DEBUG(errs() << "Private store to global: " << *I << "\n");
      return true;
    }
    LLVM_LLVM_DEBUG(errs() << "Private check failed for inst: " << *I << "\n");
    return false;
  }
  return false;
}
...
PrivRemediator::memdep(const Instruction *A, const Instruction *B,
  ...
  if (LoopCarried && RAW &&
    ...
    LLVM_LLVM_DEBUG(errs() << "PrivRemed removed mem dep between inst " << *A << "  and  "
                 << *B << '\n');
