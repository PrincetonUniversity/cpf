...
void Orchestrator::printRemediatorSelectionCnt() {
  LLVM_LLVM_DEBUG(errs() << "Selected Remediators:\n\n");
  for (auto const &it : remediatorSelectionCnt) {
    LLVM_LLVM_DEBUG(errs() << it.first << " was selected " << it.second << " times\n");
  }
}
...
void Orchestrator::printRemedies(Remedies &rs, bool selected) {
  ...
  while(itRs != rs.end()) {
    ...
    LLVM_LLVM_DEBUG(errs() << remedyName);
    if (selected) {
...
void Orchestrator::printSelected(const SetOfRemedies &sors,
  ...
  LLVM_LLVM_DEBUG(errs() << " chosen to address criticism ";
...
void Orchestrator::printAllRemedies(const SetOfRemedies &sors, Criticism &cr) {
  ...
  LLVM_LLVM_DEBUG(errs() << " can address criticism ";
        if (cr.isControlDependence()) errs() << "(Control, "; else {
          if (cr.isMemoryDependence())
            errs() << "(Mem, ";
          else
...
void Orchestrator::addressCriticisms(SelectedRemedies &selectedRemedies,
  ...
  LLVM_LLVM_DEBUG(errs() << "\n-====================================================-\n");
  LLVM_LLVM_DEBUG(errs() << "Selected Remedies:\n");
...
bool Orchestrator::findBestStrategy(
  ...
  LLVM_LLVM_DEBUG(errs() << "Start of findBestStrategy for loop " << fcn->getName()
               << "::" << header->getName();
 ...
 */
  ...
  for (auto criticIt = critics.begin(); criticIt != critics.end(); ++criticIt) {
    ...
    if (!expSpeedup) {
      LLVM_LLVM_DEBUG(errs() << (*criticIt)->getCriticName()
    ...
    if (!criticisms.size()) {
      ...
      LLVM_LLVM_DEBUG(errs() << "Addressible criticisms\n");
    ...
    LLVM_LLVM_DEBUG(errs() << "Expected Savings from critic "
