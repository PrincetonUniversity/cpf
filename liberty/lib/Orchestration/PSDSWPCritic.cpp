  ...
  bool operator()(const SCC &scc) const {
    for (auto edge : make_range(scc.begin_edges(), scc.end_edges())) {
      ...
      if (edge->isLoopCarriedDependence()) {
        ...
        // LLVM_LLVM_DEBUG(errs() << "loop-carried edge(s) found from "
...
bool checkIfRemateriazable(const DGEdge<SCC> *edge, const SCC &outgoingSCC) {
  ...
  LLVM_LLVM_DEBUG(errs() << "Remateriazable variable(s) found\n");
  return true;
}
*/
...
bool PSDSWPCritic::doallAndPipeline(const PDG &pdg, const SCCDAG &sccdag,
      ...
      */
      ...
      if (findMaxReplicableStage(pdg, sccdag, before, perf, maxReplicable,
        ...
        if (!notMaxReplicable.empty()) {
          LLVM_LLVM_DEBUG(errs() << "DOALL not applicable to " << fcn->getName()
                       << "::" << header->getName()
  ...
  LLVM_LLVM_DEBUG(errs() << "PS-DSWP not applicable to " << fcn->getName() << "::"
...
template <class GT> void writeGraph(const std::string &filename, GT *graph) {
  ...
  if (!EC) {
    ...
    LLVM_LLVM_DEBUG(errs() << "Error opening file for writing!\n");
    abort();
  }
}
...
void PSDSWPCritic::simplifyPDG(PDG *pdg) {
  ...
  for (auto edge : optimisticPDG->getEdges()) {
    if (edge->isLoopCarriedDependence()) {
      ...
      if (!edge->isRemovableDependence()) {
        LLVM_LLVM_DEBUG(errs() << "Cannot remove loop-carried ";
  ...
  for (auto edge : toBeRemovedEdges) {
    // LLVM_LLVM_DEBUG(errs() << " Removing loop-carried from " << *edge->getOutgoingT()
    //             << " to " << *edge->getIncomingT() << '\n');
...
bool PSDSWPCritic::avoidElimDep(
  ...
  if (moveFrontCost != ULONG_MAX && moveFrontCost <= moveBackCost) {
    ...
    for (Instruction *I : tmpInstsMovedToFront) {
      ...
      LLVM_LLVM_DEBUG(errs() << "Move inst to first sequential stage: " << *I << '\n');
    }
    return true;
    ...
    for (Instruction *I : tmpInstsMovedToBack) {
      ...
      LLVM_LLVM_DEBUG(errs() << "Move inst to last sequential stage: " << *I << '\n');
    }
    return true;
  }
  return false;
  ...
  for (auto *st : earlyStages) {
    ...
    for (PipelineStage::ISet::const_iterator j = st->instructions.begin(),
      ...
      if (moveBackCost != ULONG_MAX) {
        ...
        LLVM_LLVM_DEBUG(errs() << "Movable output I/O inst along with dependent insts to "
        ...
        LLVM_LLVM_DEBUG(errs()
              ...
              << *inst << '\n');
        for (auto *inst : tmpInstsMovedToBack) {
          LLVM_LLVM_DEBUG(errs() << "Part of non-movable insts: " << *inst << "\n";);
        }
        movedAllIO = false;
        break;
      }
    ...
    if (moveFrontCost != ULONG_MAX) {
      ...
      for (auto *I : tmpInstsMovedToFront) {
        ...
        if (firstStage && firstStage->type == PipelineStage::Sequential) {
          ...
          LLVM_LLVM_DEBUG(errs() << "Moved loop exit branch or dependent to loop exit "
      ...
      LLVM_LLVM_DEBUG(errs() << "\nNot movable loop exit branch inst along with "
                   ...
                   << *loopExitBr << '\n');
      for (auto *I : tmpInstsMovedToFront) {
        LLVM_LLVM_DEBUG(errs() << "Part of non-movable insts: " << *I << "\n";);
      }
    }
  }
}
...
CriticRes PSDSWPCritic::getCriticisms(PDG &pdg, Loop *loop,
  ...
  LLVM_LLVM_DEBUG(errs() << "Begin criticisms generation for PS-DSWP critic\n");
  ...
  */
  ...
  LLVM_LLVM_DEBUG(ps->dump_pipeline(errs()));
