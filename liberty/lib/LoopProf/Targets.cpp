...
Loop *header_to_loop_mapping_iterator::operator*() const
{
  ...
  LLVM_LLVM_DEBUG(errs() << "header is: " << *header);
  LLVM_LLVM_DEBUG(errs() << "function name is: " << fcn->getName() << "\n");
  LoopInfo &li = mloops.getAnalysis_LoopInfo(fcn);
  Loop *loop = li.getLoopFor(header);
  assert( loop && "Specified header not within a loop???");
  LLVM_LLVM_DEBUG(errs() << "Loop->getHeader is: " << *(loop->getHeader()));
...
bool Targets::expectsManyIterations(const Loop *loop)
{
  ...
  //LLVM_LLVM_DEBUG(errs() << "Loop edge profiling " << fname << "::" << hname << "\t totalIterations is : "  << totalIterations <<  ", totalInvocations is : " << totalInvocations <<  "\n");
...
void Targets::addLoopByName(Module &mod, const std::string &fname, const std::string &hname, unsigned long wt, bool minIterCheck)
{
  ...
  else
  {
    ...
    if( minIterCheck )
      ...
      {
        LLVM_LLVM_DEBUG(errs() << "Ignoring loop " << fname << "::" << hname << "\t(wt "<< wt <<") because too few iters/invoc\n");
        return;
      }
...
bool Targets::runOnModule(Module &mod)
{
  ...
  else
  {
    ...
    for(LoopProfLoad::iterator i=load.begin(), e=load.end(); i!=e; ++i)
    {
      ...
      if( i->second < min )
      {
        LLVM_LLVM_DEBUG(errs() << "Ignoring loop " << fname << "::" << hname << "\tTime: " << i->second << " because too little weight\n");
        continue;
      }
