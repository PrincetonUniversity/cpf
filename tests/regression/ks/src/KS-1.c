...
ReadNetList(char *fname)
{
	...
	TRY(head = prev = (Module *)malloc(sizeof(Module)),
	    prev != NULL, "ReadData",exit(1),
	    "unable to allocate a module list node");
	(*prev).module = atol(strtok(NULL, " \t\n"))-1;
	(*prev).next = NULL;
	while ((tok = strtok(NULL, " \t\n")) != NULL) {
	    TRY(node = (Module *)malloc(sizeof(Module)),
		node != NULL, "ReadData",exit(1),
		"unable to allocate a module list node");
...
NetsToModules(void)
{
	...
	for (modNode = nets[net]; modNode != NULL; modNode = (*modNode).next) {
		...
		netNode != NULL, "NetsToModules", exit(1),
		"unable to allocate net list node");
	    (*netNode).net = net;
