...
\begin{figure*}[t]
  \centering
  \scriptsize
  \subfloat[Sequential code (version 0)]
  {
  ...
  %  \end{minipage}
  %}
  \caption{Sequential code examples. Assume that the outer loop is parallelized and array \textit{b} is liveout.}
\end{figure*}
...
%    \end{minipage}
%  }
  \caption{Sequential code with speculation checks and logging, before
parallelization. Changes are in grey. Assume that the outer loop is parallelized and array \textit{b} is liveout.}
\end{figure*}
...
examples. NO: No overhead, CC: Cheap checks, EM: Expensive Memory Checks, WL:
Write Logging
  }
