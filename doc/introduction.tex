...
automatic parallelization for multicore remains deficient.
...
fewer dependences in practice than can be proved nonexistant by memory
...
presence of unrestricted pointers, an task that proved insurmountable
...
to remove memory dependences that cannot exist without memory
operations the pruned path.  In contrast, prior work systems are
unaware of these impacts and unnecessarily employ additional
speculative techniques to remove those memory dependences.

Per{\em spec}tive makes adding new specutive enablers feasible.  The
planner in combination with the speculation-aware memory analyzer
...
compared to Privateer, the most effiicent prior state-of-the-art
