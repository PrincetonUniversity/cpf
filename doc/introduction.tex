...
version.
% SIMONE: should we say, instead, that these programs either don't have CGP at all (e.g., GCC) or what they have is enough for a few cores only and it is a weak-scaling rather than strong scaling (e.g., Chrome has CGP where one tab is assigned to one thread; the more tabs, the more threads, the more CGP => weak scaling)?
...
%SIMONE: should we say, instead, that the conservativeness of static analysis forces parallelizing compilers either to parallelize small loops, which reduces the optimization coverage as well as it stresses communication demand, or to be applicable only to regular code (e.g., scientific applications)?
%
...
be enough.
% SIMONE: I like this point, but I would write it as a missing opportunity for static analysis. Something like "On top of being conservative, static analysis needs to describe program facts that are true independently with the input. This leads to missing opportunity for parallelizing compilers because programs experience phases ..."
...
% SIMONE: FYI I understand what this paragraph aims to say, but it needs some rewriting
